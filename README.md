...
const auto flexible_items {MakeFlexibleArrayField("Items", &Packet::first_item, item_count, FormatItems)};
```
...
```c++
struct Packet {
    DEFINE_PRIVATE_FIELD_WITH_PROXY(Packet, std::uint16_t, MajorMinorVerions, {0x1234})
    DEFINE_FIELD_WITH_PROXY(Packet, std::size_t, ItemCount, {10})
    DEFINE_BIT_FIELD_WITH_PROXY(Packet, MajorMinorVerions, std::uint8_t, MajorVerion, CHAR_BIT, CHAR_BIT)
