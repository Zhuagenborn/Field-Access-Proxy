...
struct Packet {
    ...
    DEFINE_PRIVATE_FIELD_WITH_PROXY(Packet, std::uint16_t, MajorMinorVerions,
                                    {default_major_minor_verions})
    DEFINE_FIELD_WITH_PROXY(Packet, String, Type, {default_type})
    DEFINE_INTEGRAL_FIELD_WITH_ENDIAN_PROXY(Packet, std::size_t, ItemCount, GetOppositeEndian(),
                                            {std::byteswap(max_items)})
    DEFINE_BIT_FIELD_WITH_PROXY(Packet, MajorMinorVerions, std::uint8_t, MajorVerion, CHAR_BIT,
                                CHAR_BIT)
    DEFINE_BIT_FIELD_WITH_PROXY(Packet, MajorMinorVerions, std::uint8_t, MinorVerion, 0, CHAR_BIT)
    DEFINE_BOOL_FIELD_WITH_PROXY(Packet, MajorMinorVerions, FirstVersionBit, IsFirstVersionBitSet,
                                 SetFirstVersionBit, 0)
    DEFINE_FLEXIBLE_ARRAY_FIELD_WITH_PROXY(Packet, Item, Items, ItemCount)
};
