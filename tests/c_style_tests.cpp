...
struct Packet {
    ...
    String type {'t', 'y', 'p', 'e'};
    std::size_t item_count {max_items};
    Item first_item[1];
};
...
const auto flexible_items {MakeFlexibleArrayField(
    "Items", &Packet::first_item, item_count,
...
TEST(CStyleFieldAccessProxy, Set) {
    ...
    {
        pkg.item_count = 0;
        vt::item_count.Set(pkg, Packet::max_items);
        EXPECT_EQ(pkg.item_count, Packet::max_items);
    }
    {
    ...
    }
    {
        ...
        EXPECT_NE(pkg.item_count, new_items.size());
...
TEST(CStyleFieldAccessProxy, Format) {
    ...
    const auto& pkg {static_cast<const Packet&>(pkg_items)};
    {
        EXPECT_EQ(vt::item_count.Format(pkg),
                  std::format("{}: {}", vt::item_count.GetName(), pkg.item_count));
    ...
    }
    {
        ...
        target << std::format("{}: {}\n", vt::item_count.GetName(), pkg.item_count);
        target << fm::FormatVersion(pkg, vt::minor_version.Get(pkg)) << '\n';
        target << fm::FormatItems(vt::flexible_items.Get(pkg)) << '\n';

        const auto fields {std::make_tuple(vt::item_count, vt::minor_version, vt::flexible_items)};
