...
public:
    ...
    std::string Format(const Struct& obj) const {
        ...
        if constexpr (!std::is_same_v<std::decay_t<Formatter>, std::nullptr_t>) {
...
public:
    ...
    const Value& Get(const Struct& obj) const noexcept {
        return obj.*field_;
    }
 ...
 * @tparam Formatter An optional callable for custom formatting.
 */
template <typename ParentFieldProxy, std::integral Target, typename Formatter = std::nullptr_t>
class BitField :
    public impl::Named,
